...
object JokeRepo {
    ...
    var jokeEndPoint: JokeEndPoint? = null

    suspend fun getJoke(): Response<JokeData>? {
        if (jokeEndPoint == null) {
            //QUESTO SI SCRIVE DOPO AVER CREATO LA FUNZIONE createRetrofitInstance()
            jokeEndPoint =createRetrofitInstance().create(JokeEndPoint::class.java )
        }
        return jokeEndPoint?.networkCall()
    }
